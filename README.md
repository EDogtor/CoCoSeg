...
基于CoCoNet架构的医疗图像分割模型 - 双独立编码器架构

## 📋 项目简介

CoCoSeg是一个专门用于CT-PET双模态医疗图像分割的深度学习模型。本项目将CoCoNet从**图像融合**模型改造为**医疗图像分割**模型，专门用于CT-PET双模态512×512像素PNG图像的分割任务。模型采用双独立编码器和中期融合策略，实现了高质量的多模态特征融合与分割。

## 🎯 模型特点

- ✅ **双独立UNet编码器**: CT和PET各一个独立的编码器，保持模态特异性
- ✅ **CAM注意力机制**: 在特征融合和跳跃连接中应用通道注意力，增强关键特征
- ✅ **InstanceNorm**: 解决小batch训练不稳定问题
...
CoCoSeg采用**双独立编码器UNet**架构，专门设计用于CT-PET多模态医疗图像分割。

### 核心组件

#### 1. 双独立UNet编码器

CT和PET分别使用独立的UNet编码器（非共享权重），提取4层特征：
- **第1层**: 32通道 (原始分辨率)
- **第2层**: 64通道 (1/2分辨率)
- **第3层**: 128通道 (1/4分辨率)
- **第4层**: 256通道 (1/8分辨率)

每个编码器独立学习各自模态的特征表示，保持模态特异性。

#### 2. CAM注意力机制

模型在以下两个位置使用CAM（Channel Attention Module）注意力：

**融合特征时**：
- 对CT和PET的编码器输出特征分别应用CAM注意力
- 自适应选择重要通道，提升融合效果

**跳跃连接中**：
- 对解码器上采样后的特征也应用CAM注意力
- 增强编码器和解码器特征之间的信息传递

CAM模块结构：
- 3个3×3卷积分支：`conv1`, `conv2`, `conv3`
- 通道注意力：`attention = softmax(query × key^T)`
- 残差连接：`output = γ × attention(conv3) + conv3`

#### 3. 中期融合策略

采用**中期融合（Mid-Level Fusion）**，在4个层级进行CT-PET特征融合：

```
256通道层级:
CT_UNet(256) + PET_UNet(256) → CAM注意力 → 融合 → 256通道

128通道层级:
CT_UNet(128) + PET_UNet(128) → CAM注意力 → 融合 → 128通道
解码器上采样(128) → CAM注意力 → 与融合特征concat

64通道层级:
CT_UNet(64) + PET_UNet(64) → CAM注意力 → 融合 → 64通道
解码器上采样(64) → CAM注意力 → 与融合特征concat

32通道层级:
CT_UNet(32) + PET_UNet(32) → CAM注意力 → 融合
解码器上采样(32) → CAM注意力 → 与融合特征concat
```

#### 4. 解码器

解码器通过上采样和跳跃连接逐步恢复分辨率：
- 每层与对应层级的融合特征concat
- 通过卷积和归一化处理特征
- 最终输出单通道分割结果
...
```
输入:
├── CT [B,1,512,512] ──> CT_UNet_Encoder ──> CT特征 (32,64,128,256ch)
│
└── PET [B,1,512,512] ──> PET_UNet_Encoder ──> PET特征 (32,64,128,256ch)

中期融合（256ch层级）:
CT(256) + PET(256) → CAM → 融合 → 256ch → 上采样

中期融合（128ch层级）:
CT(128) + PET(128) → CAM → 融合 → 128ch
解码器(128) → CAM → 与融合特征concat → 128ch → 上采样

中期融合（64ch层级）:
CT(64) + PET(64) → CAM → 融合 → 64ch
解码器(64) → CAM → 与融合特征concat → 64ch → 上采样

中期融合（32ch层级）:
CT(32) + PET(32) → CAM → 融合
解码器(32) → CAM → 与融合特征concat → 32ch

输出:
32ch → 1ch → [B,1,512,512]
```
